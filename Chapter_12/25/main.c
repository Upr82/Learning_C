    ...
    while(scanf("%d", &SIZE)==1 && SIZE>0)
    {
        ...
        while(words<SIZE)//цикл чтения и разбора из буфера ввода
        {
            ...
            if(old_c==1 && cur_c==0)
            {
                ...
                arr_s[words]=(char *)malloc(lenght+1);//выделяем память столько раз, сколько заканчивается слов
                printf("Адрес указателя на начало слова №%d = %p\n", words+1, arr_s[words]);/*для информации,
                выводим адрес начала слова для каждого слова*/
                strncpy(arr_s[words], pstart, lenght);//копируем найденное слово из строки в выделенную память
                *(arr_s[words]+lenght)='\0';//делаем из массива строку
        ...
        {
            puts(arr_s[i]);
            free(arr_s[i]);//освобождаем память для каждого указателя в массиве
        }
