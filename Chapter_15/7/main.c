#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <limits.h>

#define SOLID 0
#define DOTTED 1
#define DASHED 2

#define BLUE 4
#define GREEN 2
#define RED 1
#define BLACK 0

#define YELLOW (RED | GREEN)
#define MAGENTA (RED | BLUE)
#define CYAN (GREEN | BLUE)
#define WHITE (RED | GREEN | BLUE)

#define OPAQUE 0x1
#define FILL_RED 0x2
#define FILL_GREEN 0x4
#define FILL_BLUE 0x8
#define FILL_MASK 0xE

#define BORDER 0x100
#define BORDER_RED 0x200
#define BORDER_GREEN 0x400
#define BORDER_BLUE 0x800
#define BORDER_MASK 0xE00

#define B_SOLID 0
#define B_DOTTED 0x1000
#define B_DASHED 0x2000
#define STYLE_MASK 0x3000

const char *colors[8]={"черный","красный",
"зеленый","желтый","синий","пурпурный","голубой","белый"};

struct box_props {
unsigned short opaque :1;
unsigned short fill_color :3;
unsigned short :4;
unsigned short show_border :1;
unsigned short border_color :3;
unsigned short border_style :2;
unsigned short :2;
};

union Views {
struct box_props st_view;
unsigned short us_view;
};

void show_settings(const struct box_props *pb);
void show_settings1(unsigned short);
char *itobs(unsigned short n, char *ps);

int main()
{
    system("chcp 1251");

    union Views box={{true, YELLOW, true, GREEN, DASHED}};
    char bin_str[8*sizeof(unsigned short)+1];

    printf("Исходные настройки окна:\n");
    show_settings(&box.st_view);
    printf("Настройки окна с использованием представления unsigned short: \n");
    show_settings1(box.us_view);

    printf("Комбинация битов %s\n",
           itobs(box.us_view, bin_str));
    box.us_view ^= OPAQUE;
    box.us_view &= ~FILL_MASK;
    box.us_view |= (FILL_BLUE | FILL_GREEN);
    box.us_view ^= BORDER;
    box.us_view &= ~BORDER_MASK;
    box.us_view |= (BORDER_RED | BORDER_GREEN);
    box.us_view &= ~STYLE_MASK;
    box.us_view |= B_DOTTED;

    printf("\nИзменённые настройки окна:\n");
    show_settings(&box.st_view);
    printf("Настройки окна с использованием представления unsigned short: \n");
    show_settings1(box.us_view);

    printf("Комбинация битов %s\n",
           itobs(box.us_view, bin_str));

    printf("\nEnd!\n");
    return 0;
}

void show_settings(const struct box_props *pb)
{
    printf("Окно %s\n", pb->opaque==true ?"непрозрачно":"прозрачно");
    printf("Цвет фона %s\n", colors[pb->fill_color]);
    printf("Рамка %s\n", pb->show_border==true? "отображается" : "не отображается");
    printf("Цвет рамки %s\n", colors[pb->border_color]);
    printf("Стиль рамки ");
    switch (pb->border_style)
    {
    case SOLID: printf("сплошной\n");break;
    case DOTTED: printf("пунктирный\n");break;
    case DASHED: printf("штриховой\n");break;
    default : printf("неизвестно.\n");
    }
}
void show_settings1(unsigned short us)
{
    printf("Окно %s.\n",
           (us & OPAQUE)==OPAQUE? "непрозрачно" : "прозрачно");
    printf("Цвет фона: %s.\n",
           colors[(us>>1) & 0x7]);
    printf("Рамка %s.\n",
           (us & BORDER)==BORDER? "отображается": "не отображается");
    printf("Цвет рамки %s\n",
           colors[(us >>9) & 0x7]);
    printf("Стиль рамки ");
    switch (us & STYLE_MASK)
    {
        case B_SOLID: printf("сплошной.\n"); break;
        case B_DOTTED: printf("пунктирный\n"); break;
        case B_DASHED: printf("штриховой\n"); break;
        default : printf("неизвестно.\n");
    }
}
char *itobs(unsigned short n, char *ps)
{
    int i;
    const static int size = CHAR_BIT * sizeof(unsigned int);

    for(i=size-1; i>=0;i--, n>>=1)
        ps[i]=(n & 0x1)+'0';
    ps[size]='\0';

    return ps;
}
